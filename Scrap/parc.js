...
function readFormData() {
  var formData = {};
  formData["fullName"] = document.querySelectorAll(".fullName").value;
  console.log(formData);
  return formData;
}

function insertNewRecord(data) {
  var able = document.querySelectorAll(".employeeList");
  able.forEach((able) => {
    const table = able.getElementsByTagName("tbody")[0];
...
function resetForm() {
  document.querySelectorAll(".fullName").value = "";
...
function onEdit(td) {
  ...
  const edit = document.querySelectorAll(".fullName");
  edit.forEach((edit) => {
    edit.value = selectedRow.cells[0].innerHTML;
  });
}
...
function onDelete(td) {
  if (confirm("Are you sure to delete this record ?")) {
    ...
    const lip = document.querySelectorAll(".employeeList");
    lip.forEach((lip) => {
      lip.deleteRow(row.rowIndex);
      resetForm();
    });
  }
}
function validate() {
  isValid = true;
  const flnm = document.querySelectorAll(".fullName");
  const axe = document.querySelectorAll(".fullNameValidationError");
  flnm.forEach((flnm) => {
    if (flnm.value == "") {
      isValid = false;
      axe.forEach((axe) => {
        axe.classList.remove("hide");
      });
    } else {
      isValid = true;
      axe.forEach((axe) => {
        if (!axe.classList.contains("hide")) axe.classList.add("hide");
      });
    }
    return isValid;
  });
...
//Comments form API
async function getNewComments(id) {
  let response = await fetch(`https://dummyjson.com/comments/1`, {
    method: "GET",
  });
  if (!response.ok) {
        ...
        const HTML = `
                    <div class="head">
                      <div class="user">
                        <div class="profile-picture">
                          <img src="./images/profile-${
                            allPosts.posts[j].id + 1
                          }.jpg" alt="Profile Picture" />
                        </div>
                        <div class="ingo">
                          <h3>Beta${[j]}</h3>
                          <small>${allPosts.posts[j].title}</small>
                        </div>
                      </div>
                      <span class="edit">
                        <i class="uil uil-ellipsis-h"></i>
...
//       commentSection[j].classList.toggle("comment-section");
//     }
//   });
// }
